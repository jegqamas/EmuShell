...
void Mapper045::WriteSRM(uint16_t& address, uint8_t& value)
{
    if (locked)
    ...
    else
    {
        ...
        {
            // chr_and = 0xFF >> ~S_bits;
            //chr_and = 0xFF >> ~(value & 0xF);
            if ((value & 0x8) == 0x8)
                chr_and = ((1 << ((value & 0x7) + 1)) - 1);
            else
                chr_and = ((value > 0) ? 0 : ~0);
            chr_or = (chr_or & 0x00FF) | ((value & 0xF0) << 4);
            SetupCHR();
            break;
        }
        case 3:
        {
            locked = (value & 0x40) == 0x40;
            prg_and = (value & 0x3F) ^ 0x3F;
            SetupPRG();
            break;
        }
        }
...
void Mapper045::WritePRG(uint16_t& address, uint8_t& value)
{
...
void Mapper045::OnPPUA12RaisingEdge()
{
...
void Mapper045::SaveState()
{
    ...
    state_str->Write( locked);
    state_str->Write( regCounter);
}
void Mapper045::LoadState()
{
    Board::LoadState();
    ...
    state_str->Read( locked);
    state_str->Read( regCounter);
}
}
