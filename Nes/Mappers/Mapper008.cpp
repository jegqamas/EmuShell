...
void Mapper008::WriteEXP(uint16_t& address, uint8_t& value)
{
    switch (address)
    {
    case 0x4501:
...
void Mapper008::WritePRG(uint16_t& address, uint8_t& value)
{
    Switch32KPRG((value >> 4) & 0x3, true);
    Switch08KCHR(value & 0x3, chr_01K_rom_count > 0);
}
void Mapper008::OnCPUClock()
{
    if (irqEnable)
    {
        irqCounter++;
