...
void Mapper192::WritePRG(uint16_t& address, uint8_t& value)
{
...
void Mapper192::SetupCHR()
{
    if (!flag_c)
    {
        isRam = chr_reg[0] >= 0x8 && chr_reg[0] <= 0xB;
        Switch02KCHR(isRam ? (chr_reg[0] - 0x8) >> 1 : chr_reg[0] >> 1, 0x0000, !isRam);
        isRam = chr_reg[1] >= 0x8 && chr_reg[1] <= 0xB;
        Switch02KCHR(isRam ? (chr_reg[1] - 0x8) >> 1 : chr_reg[1] >> 1, 0x0800, !isRam);
        isRam = chr_reg[2] >= 0x8 && chr_reg[2] <= 0xB;
        Switch01KCHR(isRam ? (chr_reg[2] - 0x8) : chr_reg[2], 0x1000, !isRam);
        isRam = chr_reg[3] >= 0x8 && chr_reg[3] <= 0xB;
        Switch01KCHR(isRam ? (chr_reg[3] - 0x8) : chr_reg[3], 0x1400, !isRam);
        isRam = chr_reg[4] >= 0x8 && chr_reg[4] <= 0xB;
        Switch01KCHR(isRam ? (chr_reg[4] - 0x8) : chr_reg[4], 0x1800, !isRam);
        isRam = chr_reg[5] >= 0x8 && chr_reg[5] <= 0xB;
        Switch01KCHR(isRam ? (chr_reg[5] - 0x8) : chr_reg[5], 0x1C00, !isRam);
    }
    else
    {
        isRam = chr_reg[0] >= 0x8 && chr_reg[0] <= 0xB;
        Switch02KCHR(isRam ? (chr_reg[0] - 0x8) >> 1 : chr_reg[0] >> 1, 0x1000, !isRam);
        isRam = chr_reg[1] >= 0x8 && chr_reg[1] <= 0xB;
        Switch02KCHR(isRam ? (chr_reg[1] - 0x8) >> 1 : chr_reg[1] >> 1, 0x1800, !isRam);
        isRam = chr_reg[2] >= 0x8 && chr_reg[2] <= 0xB;
        Switch01KCHR(isRam ? (chr_reg[2] - 0x8) : chr_reg[2], 0x0000, !isRam);
        isRam = chr_reg[3] >= 0x8 && chr_reg[3] <= 0xB;
        Switch01KCHR(isRam ? (chr_reg[3] - 0x8) : chr_reg[3], 0x0400, !isRam);
        isRam = chr_reg[4] >= 0x8 && chr_reg[4] <= 0xB;
        Switch01KCHR(isRam ? (chr_reg[4] - 0x8) : chr_reg[4], 0x0800, !isRam);
        isRam = chr_reg[5] >= 0x8 && chr_reg[5] <= 0xB;
        Switch01KCHR(isRam ? (chr_reg[5] - 0x8) : chr_reg[5], 0x0C00, !isRam);
    }
}
void Mapper192::SetupPRG()
{
...
void Mapper192::OnPPUA12RaisingEdge()
{
...
void Mapper192::SaveState()
{
    ...
    state_str->Write(isRam);

}
void Mapper192::LoadState()
{
    Board::LoadState();
