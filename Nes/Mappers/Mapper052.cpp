...
void Mapper052::WriteSRM(uint16_t& address, uint8_t& value)
{
    if (!locked)
    {
        ...
        {
            locked = true;
            prg_and = (value << 1 & 0x10) ^ 0x1F;
            prg_or = ((value & 0x6) | (value >> 3 & value & 0x1)) << 4;
            chr_and = ((value & 0x40) << 1) ^ 0xFF;
            chr_or = ((value >> 3 & 0x4) | (value >> 1 & 0x2) | ((value >> 6) & (value >> 4) & 0x1)) << 7;
            SetupCHR();
            SetupPRG();
        }
    }
...
void Mapper052::WritePRG(uint16_t& address, uint8_t& value)
{
...
void Mapper052::OnPPUA12RaisingEdge()
{
