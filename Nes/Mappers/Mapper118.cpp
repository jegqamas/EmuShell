...
void Mapper118::WritePRG(uint16_t& address, uint8_t& value)
{
...
uint8_t Mapper118::ReadNMT(uint16_t& address)
{
    switch ((address >> 10) & 3)
    {
    case 0:
        return nmt_banks[(chr_reg[flag_c ? 2 : 0] >> 7) & 1][address & 0x3FF];
    case 1:
        return nmt_banks[(chr_reg[flag_c ? 3 : 0] >> 7) & 1][address & 0x3FF];
    case 2:
        return nmt_banks[(chr_reg[flag_c ? 4 : 1] >> 7) & 1][address & 0x3FF];
    case 3:
        return nmt_banks[(chr_reg[flag_c ? 5 : 1] >> 7) & 1][address & 0x3FF];
    }
    return 0;// Make compiler happy !
}
void Mapper118::WriteNMT(uint16_t& address, uint8_t& value)
{
    switch ((address >> 10) & 3)
    {
    case 0:
        nmt_banks[(chr_reg[flag_c ? 2 : 0] >> 7) & 1][address & 0x3FF] = value;
        break;
    case 1:
        nmt_banks[(chr_reg[flag_c ? 3 : 0] >> 7) & 1][address & 0x3FF] = value;
        break;
    case 2:
        nmt_banks[(chr_reg[flag_c ? 4 : 1] >> 7) & 1][address & 0x3FF] = value;
        break;
    case 3:
        nmt_banks[(chr_reg[flag_c ? 5 : 1] >> 7) & 1][address & 0x3FF] = value;
        break;
    }
}
void Mapper118::SetupCHR()
{
    if (!flag_c)
    {
...
void Mapper118::OnPPUA12RaisingEdge()
{
