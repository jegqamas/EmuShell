...
void Mapper049::WriteSRM(uint16_t& address, uint8_t& value)
{
    if (prg_enabled[prg_indexes[0]] && prg_isWritable[prg_indexes[0]])
    {
        prg_32Mode = (value & 1) == 1;
        prg_or = (value >> 2) & 0x30;
        chr_or = (value & 0xC0) << 1;
        prg_32Page = (value >> 4) & 0x3;
        SetupCHR();
        SetupPRG();
    }
}
void Mapper049::WritePRG(uint16_t& address, uint8_t& value)
{
...
void Mapper049::SetupPRG()
{
    ...
    {
        Switch32KPRG(prg_32Page, true);
    }
}
void Mapper049::OnPPUA12RaisingEdge()
{
...
void Mapper049::SaveState()
{
    ...
    state_str->Write(prg_32Mode);
    state_str->Write( prg_32Page);
    for (int i=0; i<6; i++)
...
void Mapper049::LoadState()
{
    ...
    state_str->Read(prg_32Mode);
    state_str->Read( prg_32Page);
