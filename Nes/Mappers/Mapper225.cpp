...
{
    this->Name = "Unknown";
    this->MapperNumber = 225;
} void Mapper225::HardReset()
{
    Board::HardReset();
    for (int i=0; i<4; i++)
        RAM[i]=0;
}
void Mapper225::WriteEXP(uint16_t& address, uint8_t& value)
{
    if (address >= 0x5800)
        RAM[address & 0x3] = (uint8_t)(value & 0xF);
}
uint8_t Mapper225::ReadEXP(uint16_t& address)
{
    if (address >= 0x5800)
        return RAM[address & 0x3];
    return 0;
}
void Mapper225::WritePRG(uint16_t& address, uint8_t& value)
{
...
void Mapper225::SaveState()
{
    Board::SaveState();
    for (int i=0; i<4; i++)
        state_str->Write(RAM[i]);
}
void Mapper225::LoadState()
{
    Board::LoadState();
    for (int i=0; i<4; i++)
