...
void Mapper095::WritePRG(uint16_t& address, uint8_t& value)
{
...
uint8_t Mapper095::ReadNMT(uint16_t& address)
{
    if (flag_c)
    {
    ...
    else
    {
        ...
        case 0:
        case 1:
            return nmt_banks[(chr_reg[0] & 0x20) >> 5][address & 0x3FF];
        case 2:
        case 3:
            return nmt_banks[(chr_reg[1] & 0x20) >> 5][address & 0x3FF];
        }
    }
    return 0;// Make the compiler happy !!
}
void Mapper095::WriteNMT(uint16_t& address, uint8_t& value)
{
    if (flag_c)
    {
        nmt_banks[(chr_reg[((address >> 10) & 3) + 2] & 0x20) >> 5][address & 0x3FF] = value;
    }
    else
    {
        switch ((address >> 10) & 3)
        ...
        case 0:
        case 1:
            nmt_banks[(chr_reg[0] & 0x20) >> 5][address & 0x3FF] = value;
            break;
        case 2:
        case 3:
            nmt_banks[(chr_reg[1] & 0x20) >> 5][address & 0x3FF] = value;
            break;
        }
    }
}
void Mapper095::SetupCHR()
{
    if (!flag_c)
    {
...
void Mapper095::OnPPUA12RaisingEdge()
{
