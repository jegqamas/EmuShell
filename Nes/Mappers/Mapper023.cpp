...
{
    this->Name = "VRC2";
    this->MapperNumber = 23;
}
void Mapper023::HardReset()
{
    Board::HardReset();
...
void Mapper023::WriteSRM(uint16_t& address, uint8_t& value)
{
    if (address == 0x6000)
        security = (value & 0x1);
}
uint8_t Mapper023::ReadSRM(uint16_t& address)
{
    if (address == 0x6000)
        return security;
    return 0;
}
void Mapper023::WritePRG(uint16_t& address, uint8_t& value)
{
    switch (address)
    {
    case 0x8000:
...
void Mapper023::SaveState()
{
    Board::SaveState();
    for (int i=0; i<8; i++)
        state_str->Write(chr_reg[i]);
    state_str->Write(security);
}
void Mapper023::LoadState()
{
    Board::LoadState();
    for (int i=0; i<8; i++)
        state_str->Read(chr_reg[i]);
    state_str->Read(security);
}
}
