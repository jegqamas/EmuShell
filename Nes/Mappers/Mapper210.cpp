...
{
    this->Name = "N106";
    this->MapperNumber = 210;
}
void Mapper210::HardReset()
{
    Board::HardReset();
    Switch08KPRG(prg_08K_rom_mask, 0xE000, true);
    enable_mirroring_switch = false;
}
void Mapper210::WriteEXP(uint16_t& address, uint8_t& value)
{
...
void Mapper210::WritePRG(uint16_t& address, uint8_t& value)
{
    switch (address & 0xF800)
    {
    case 0x8000:
...
uint8_t Mapper210::ReadEXP(uint16_t& address)
{
    switch (address & 0xF800)
    {
    case 0x5000:
...
void Mapper210::WriteNMT(uint16_t& address, uint8_t& value)
{
    if (enable_mirroring_switch)
    {
...
uint8_t Mapper210::ReadNMT(uint16_t& address)
{
    if (enable_mirroring_switch)
    {
        temp_nmt = nmt_indexes[(address >> 10) & 0x3];
